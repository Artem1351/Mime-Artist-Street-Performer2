...
class TugboatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_tugboat)
    ...
    private fun setupViews() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(
            findViewById(R.id.tugboatGameScene)
        ) { v, insets ->
    ...
    private fun onActionSuccess() {
        ...
        val nextPhase = when (currentPhase) {
            ...
            GamePhase.HOLD -> GamePhase.SWIPE_RIGHT
            GamePhase.SWIPE_RIGHT -> GamePhase.PREPARE_HOLD_1
            GamePhase.PREPARE_HOLD_1 -> GamePhase.HOLD_1
            GamePhase.HOLD_1 -> GamePhase.SWIPE_LEFT
            GamePhase.SWIPE_LEFT -> GamePhase.PREPARE_HOLD_2
    ...
    private fun updateHintAnimated(newText: String) {
        val newIconRes = when (currentPhase) {
            GamePhase.SWIPE_LEFT, GamePhase.SWIPE_RIGHT -> R.drawable.ic_swipe_horizontal
    ...
    private fun animateIconChange(newIconRes: Int) {
        // Если иконка не изменилась, ничего не делаем
        ...
        fadeOut.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(animation: Animator) {
                // Когда старая иконка исчезла, меняем изображение
                hintIcon.setImageResource(newIconRes)

                // И плавно показываем новую иконку
